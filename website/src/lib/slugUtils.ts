...
export function findTrackByFolderAndSlug(
  ...
  const candidates = tracks.filter(track => {
    ...
    console.log(`findTrackByFolderAndSlug: Checking "${track.name}" in folder "${track.folder || 'root'}":`, {
      trackSlug: trackNameToSlug(track.name),
      trackMatches,
      folderMatches,
      overall: trackMatches && folderMatches
    });
  ...
  if (candidates.length === 1) {
    console.log('findTrackByFolderAndSlug: Found exact match:', candidates[0].name);
    return candidates[0];
