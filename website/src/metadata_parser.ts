// Type definitions for metadata parsing
...
/**
 * Extracts metadata from code comments
 * @param raw_code - The source code to parse
 * @returns Object containing extracted metadata tags
 */
export function getMetadata(raw_code: string | null | undefined): MetadataTags {
  if (raw_code == null) {
    ...
    for (const tag_match of tag_matches) {
      ...
      if (ALLOW_MANY.includes(tag)) {
        const tag_list = tag_value
          ...
          .map((t) => t.trim())
          .filter((t) => t !== '');
        tags[tag] = tag in tags ? (tags[tag] as string[]).concat(tag_list) : tag_list;
      } else {
        tag_value = tag_value.replaceAll(/\s+/g, ' ');
        tags[tag] = tag in tags ? (tags[tag] as string) + ' ' + tag_value : tag_value;
      }
    }
  }
