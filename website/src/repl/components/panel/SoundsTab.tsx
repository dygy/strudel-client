...
import { confirmDialog } from '@src/repl/util';
import { clearIDB, userSamplesDBConfig } from '@src/repl/idbutils';
import { prebake } from '@src/repl/prebake';

interface SoundData {
  type: 'sample' | 'synth' | 'soundfont' | 'wavetable';
  tag?: string;
  prebake?: boolean;
  samples?: any[] | Record<string, any>;
  tables?: any[] | Record<string, any>;
  fonts?: any[];
}

interface Sound {
  data: SoundData;
  onTrigger: (time: number, params: any, onended?: () => void) => Promise<any>;
}

type SoundMap = Record<string, Sound>;

const getSamples = (samples: any[] | Record<string, any> | undefined): number => {
  if (!samples) return 0;
  return Array.isArray(samples) ? samples.length : typeof samples === 'object' ? Object.values(samples).length : 1;
};
  ...
  return (
    <div id="sounds-tab" className="px-4 flex gap-2 flex-col w-full h-full text-foreground">
      ...
      <div className=" flex shrink-0 flex-wrap">
        <ButtonGroup
          ...
          items={{
            [soundFilterType.SAMPLES]: 'samples',
            [soundFilterType.DRUMS]: 'drum-machines',
            [soundFilterType.SYNTHS]: 'Synths',
            [soundFilterType.WAVETABLES]: 'Wavetables',
            [soundFilterType.USER]: 'User',
            [soundFilterType.IMPORT_SOUNDS]: 'import-sounds',
          }}
        ></ButtonGroup>
      </div>
        ...
        {soundEntries.map(([name, { data, onTrigger }]) => {
          return (
            ...
            >
              ...
              {data?.type === 'soundfont' ? `(${data.fonts?.length || 0})` : ''}
            </span>
          );
        })}
        {!soundEntries.length && soundsFilter === soundFilterType.IMPORT_SOUNDS ? (
   ...
   └─ smash
      ...
      └─ smashmiddle.wav`}
        ...
        {!soundEntries.length && soundsFilter !== soundFilterType.IMPORT_SOUNDS ? 'No sounds loaded' : ''}
      </div>
    </div>
  );
}