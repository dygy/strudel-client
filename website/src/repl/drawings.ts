// Type definition for drawing patterns
export type DrawingPattern = string;

export const lightflower: DrawingPattern = `Pattern.prototype.nest = function(n, cycles) {
  n = reify(n)
...
export const spiralflower: DrawingPattern = `let {innerWidth:ww,innerHeight:wh} = window;
...
export const syncexample: DrawingPattern = `"<0 1 2 3>/2"
.off(1/2, add(4))
.off(1, add(2))
...
export const moveRescaleZoom: DrawingPattern = `
...
export const strudelS: DrawingPattern = `
const rescale = register('rescale', function (f, pat) {
  return pat.mul(x(f).w(f).y(f).h(f));
})
