...
The Strudel REPL includes several keyboard shortcuts to enhance the live coding experience:

### Code Evaluation
- **Ctrl+Enter** (or **Cmd+Enter** on Mac): Evaluate/update the current pattern
- **Ctrl+U** (or **Cmd+U** on Mac): Update the current pattern (alternative to Ctrl+Enter)

### Playback Control
- **Ctrl+P** (or **Cmd+P** on Mac): Toggle play/pause

### File Management
- **Ctrl+S** (or **Cmd+S** on Mac): Save the current track (when authenticated)

### Editor Features
The editor also supports different keybinding modes that can be configured in Settings:
...
These shortcuts are designed to minimize interruption to the creative flow during live coding sessions, allowing quick pattern updates and playback control without reaching for the mouse.
